...
import { preProcess, postProcess } from './CommonsApi';
...
export async function buildOptions(payload) {
...
async function performApiCall(payload) {
  const options = await buildOptions(payload);
...
export async function getReservedParameterValue(name, endpoint, requestParameters) {
...
export async function getReservedParameters(endpoint, requestParameters) {
  const _path = await getReservedParameterValue('_path', endpoint, requestParameters);
  const _times = await getReservedParameterValue('_times', endpoint, requestParameters);
  const _type = await getReservedParameterValue('_type', endpoint, requestParameters);
  const _gasPrice = await getReservedParameterValue('_gasPrice', endpoint, requestParameters);
...
export async function processSuccessfulApiCall(payload, rawResponse) {
  ...
  const postProcessApiCallResponse = await postProcess(rawResponse, endpoint, aggregatedApiCall.parameters, {
...
export async function callApiWithAdapter(payload) {
  ...
  const processedParameters = await preProcess(endpoint, parameters);
