  ...
  (exports.modules = {
    27921: (e, r) => {
      ...
      function normalizeRepeatedSlashes(e) {
        let r = e.split('?'),
          t = r[0];
        return t.replace(/\\/g, '/').replace(/\/\/+/g, '/') + (r[1] ? '?' + r.slice(1).join('?') : '');
      }
      async function loadGetInitialProps(e, r) {
        let t = r.res || (r.ctx && r.ctx.res);
        if (!e.getInitialProps)
          return r.ctx && r.Component ? { pageProps: await loadGetInitialProps(r.Component, r.ctx) } : {};
        let n = await e.getInitialProps(r);
        if (t && isResSent(t)) return n;
        if (!n) {
          let r =
            '"' + getDisplayName(e) + '.getInitialProps()" should resolve to an object. But found "' + n + '" instead.';
          throw Error(r);
        }
        return n;
      }
